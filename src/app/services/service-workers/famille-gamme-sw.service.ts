...
import {
    FamilleGammeApiService
} from '../api/famille-gamme-api.service';
import {
    IndexedDbService
...
import {
    FamilleGamme
} from 'src/app/classes/famille-gamme';
import {
    Plan
...
export class FamilleGammeSwService {

    idb: Dexie.Table<FamilleGamme, number>;

    constructor(private connectivity: ConnectivityService,
        private api: FamilleGammeApiService,
        private idbService: IndexedDbService,
        private deferredQueries: DeferredQueriesService) {
        this.idb = this.idbService.famillesGamme;
    }
    ...
    /// GET ALL
    ///
    getAll(): Promise<FamilleGamme[]> {

        // On prépare le résultat qui serra retourné dans la promesse
        let result: Promise<FamilleGamme[]>;
            ...
            this.connectivity.isConnected.then(isConnected => {
                ...
                if (isConnected) {
                    ...
                    result = new Promise(rslv => {
                        ...
                        this.api.getAll().subscribe((famillesGamme: FamilleGamme[]) => {

                            // On vide la base locale
                            this.idb.clear();

                            // On ajoute à l'IDB les données obtenue
                            famillesGamme.forEach((familleGamme, index) => {
                                this.idb.add(familleGamme);
                                famillesGamme[index].gammes = familleGamme.gammes.map(gamme => Object.assign(Gamme.newEmpty(), gamme));
                            });
                    ...
                    result = new Promise(rslv => {
                        ...
                        this.idb.toArray().then(famillesGamme => {
                            famillesGamme.forEach((familleGamme, index) => {
                                this.idb.add(familleGamme);
                                famillesGamme[index].gammes = familleGamme.gammes.map(gamme => Object.assign(Gamme.newEmpty(), gamme));
                            });
    ...
    /// GET ONE
    ///
    get(id: number): Promise<FamilleGamme> {
        ...
        return new Promise(rtrn => {
            ...
            this.connectivity.isConnected.then(isConnected => {
                if (isConnected) {
                    ...
                    result = new Promise(rslv => {
                        this.api.get(id).subscribe((familleGamme: FamilleGamme) => {

                            familleGamme.gammes = familleGamme.gammes.map(gamme => Object.assign(Gamme.newEmpty(), gamme));

                            // Avec la nouvelle données, on ajoute/modifie l'enregistrement
                            this.idb.put(familleGamme);
                    ...
                    result = new Promise(rslv => {
                        // Si on ne peux pas toucher l'API on call simplement l'IDB
                        this.idb.get(id).then(familleGamme => {
                            familleGamme.gammes = familleGamme.gammes.map(gamme => Object.assign(Gamme.newEmpty(), gamme));
    ...
    /// ADD
    ///
    add(familleGamme: FamilleGamme): Promise<FamilleGamme> {
            ...
            this.connectivity.isConnected.then(isConnected => {
                ...
                if (isConnected) {
                    ...
                    result = new Promise(rslv => {
                        ...
                        this.api.add(familleGamme).subscribe((added: FamilleGamme) => {

                            // On ajoute aussi à l'IDB
                            this.idb.add(added);

                            added.gammes = familleGamme.gammes;
                            ...
                            // Si on détecte une erreur, on attend un changement de connexion et on réessaye
                            this.connectivity.event.subscribe(connected => rslv(this.add(familleGamme)));
                    ...
                    result = new Promise(rslv => {
                        ...
                        this.idb.orderBy('id').reverse().first().then(lastRecord => {
                            ...
                            familleGamme.id = nextId;

                            // On ajoute une requête différée pour update la base plus tard
                            this.deferredQueries.add(new DeferredQuery(familleGamme, 'add', 'familleGamme'));

                            result = this.idb.add(familleGamme);
                            rslv(familleGamme);
    ...
    /// EDIT
    ///
    edit(familleGamme: FamilleGamme): Promise<any> {
            ...
            this.connectivity.isConnected.then(isConnected => {
                ...
                if (isConnected) {
                    ...
                    result = new Promise(rslv => {
                        ...
                        this.api.edit(familleGamme).subscribe(() => {

                            // On met à jour l'enregistrement dans l'IDB
                            this.idb.update(familleGamme.id, {
                                ...familleGamme
                            });
                            ...
                            // Si on détecte une erreur, on attend un changement de connexion et on réessaye
                            this.connectivity.event.subscribe(connected => rslv(this.edit(familleGamme)));
                    ...
                    // On met à jour l'enregistrement dans l'IDB
                    result = this.idb.update(familleGamme.id, {
                        ...familleGamme
                    });

                    // On ajoute une requête différée pour update la base plus tard
                    this.deferredQueries.add(new DeferredQuery(familleGamme, 'edit', 'familleGamme'));
                }
            ...
            this.connectivity.isConnected.then(isConnected => {
                ...
                if (isConnected) {
                    ...
                    result = new Promise(rslv => {
                        ...
                        // On delete en base
                        this.api.delete(Object.assign(FamilleGamme.newEmpty(), {
                            id
                        })).subscribe(() => {
